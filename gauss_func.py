...
def propagator(dt, a_n, u_list, R_list, S_list, e, ω):
    ...
    k1 = a_dot_RSW(a_n, e, ω, u_n, R_n, S_n)
    k1 = 2 * dt * k1
    k2 = a_dot_RSW(a_n + k1 / 2, e, ω, u_np1, R_np1, S_np1)
    k2 = 2 * dt * k2
    k3 = a_dot_RSW(a_n + k2 / 2, e, ω, u_np1, R_np1, S_np1)
    k3 = 2 * dt * k3
    k4 = a_dot_RSW(a_n + k3, e, ω, u_np2, R_np2, S_np2)
    k4 = 2 * dt * k4
    
    a_np2 = a_n + (k1 + k4) / 6 + (k2 + k3) / 3
    return(a_np2)

def integrator(a_0, e, ω, u_data, acc_R_data, acc_S_data):
    ...
    # starting point
    t_0 = u_data[0, 0] 
    a_dot_0 = a_dot_RSW(a_0, e, ω, u_list_list[0], R_list_list[0], S_list_list[0])
    ...
    for i in range(0, (len(u_data) - 1)// 2):
        ...
        a_np2 = propagator(dt, a_n, u_list, R_list, S_list, e, ω)
        
        a_dot_np2 = a_dot_RSW(a_np2, e, ω, u_list[-1], R_list[-1], S_list[-1])
...
def master_integrator(u_data, path_ele_osc, path_PCA, mjd_interval):
    """INPUT:
    ...
    ele_osc_data = np.loadtxt(path_ele_osc)
