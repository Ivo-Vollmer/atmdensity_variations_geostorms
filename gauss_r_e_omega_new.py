...
def a_dot_RSW_v2(a, e, ω, r, u, R, S):
...
def propagator_v2(dt, a_n, u_list, R_list, S_list, e_list, omega_list, r_list):
    ...
    k1 = a_dot_RSW_v2(a_n, e_n, omega_n, r_n, u_n, R_n, S_n)
    k1 = 2 * dt * k1
    k2 = a_dot_RSW_v2(a_n + k1 / 2, e_np1, omega_np1, r_np1, u_np1, R_np1, S_np1)
    k2 = 2 * dt * k2
    k3 = a_dot_RSW_v2(a_n + k2 / 2, e_np1, omega_np1, r_np1, u_np1, R_np1, S_np1)
    k3 = 2 * dt * k3
    k4 = a_dot_RSW_v2(a_n + k3, e_np2, omega_np2, r_np2, u_np2, R_np2, S_np2)
    k4 = 2 * dt * k4
    
    a_np2 = a_n + (k1 + k4) / 6 + (k2 + k3) / 3
    return(a_np2)

def integrator_v2(a_0, e, ω,r, u_data, acc_R_data, acc_S_data):
    ...
    # starting point
    t_0 = u_data[0, 0] 
    a_dot_0 = a_dot_RSW_v2(a_0, e_list_list[0], omega_list_list[0], r_list_list[0], u_list_list[0], R_list_list[0], S_list_list[0])
    ...
    for i in range(0, (len(u_data) - 1)// 2):
        ...
        a_np2 = propagator_v2(dt, a_n, u_list, R_list, S_list, e_list, omega_list, r_list)
        
        a_dot_np2 = a_dot_RSW_v2(a_np2, e_list[-1], omega_list[-1], r_list[-1], u_list[-1], R_list[-1], S_list[-1])
...
def master_integrator_v2(u_data, a_data, e_data, r_data, omega_data, path_PCA, mjd_interval):
    """INPUT:
    ...
    a_int_data, a_dot_data = integrator_v2(a_0, e, ω, r, u_data, acc_R_data, acc_S_data)
    return(a_int_data, a_dot_data)
